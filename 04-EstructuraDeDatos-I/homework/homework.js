...
function nFibonacciI(n) {
  let fibonnaci = [0, 1]
  for (let i = 2; i <= n; i++)
    fibonnaci.push(fibonnaci[i - 2] + fibonnaci[i - 1])
  return fibonnaci[n];
}
...
function decimalABinario(n){
  if (n == 1)
    ...
    return decimalABinario(parseInt(n / 2)) + (n % 2).toString();
}
...
function BinarioADecimal(n){
  if (n.length == 1)
    ...
    return parseInt((2 ** (n.length - 1)) * n.charAt(0)) + parseInt(BinarioADecimal(n.substring(1)));
}
